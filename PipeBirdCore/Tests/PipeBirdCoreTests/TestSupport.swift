...
enum Fixtures {
    ...
    static func config() -> GameConfig {
        .standard
    }

    static func freshRNG(_ c: GameConfig = .standard) -> DeterministicRNG {
        DeterministicRNG(seed: c.seed)
    }

    // MARK: states

    static func ready(_ c: GameConfig = .standard) -> GameState {
        .initial(config: c)
    }
    ...
    static func straddlingPipeState(
        birdY: Double,
    ...
    /// Run `PipeBirdEngine.step` `count` times from `state`, threading ONE rng. Returns final state + all
    /// events.
    static func steps(
        _ count: Int,
    ...
    @discardableResult
    static func drive(
        _ sim: inout PipeBirdSimulation,
        frames: Int,
        ...
        for i in 0 ..<
            frames
        {
            events.append(contentsOf: sim.advance(realDelta: realDelta, input: input(i)))
        }
        return events
    }
